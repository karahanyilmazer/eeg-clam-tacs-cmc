...
from misc.LSLStreamInfoInterface import find_stream
from misc.xdf2mne import stream2raw, marker_stream2events
...
def get_ssd_forward_model(raw): #SSD forward model - SNR
    ...
    data_alpha = raw.copy().filter(10,14,l_trans_bandwidth=1, h_trans_bandwidth=1)._data #
    data_broad = raw.copy().filter(1,30,l_trans_bandwidth=1, h_trans_bandwidth=1)._data #,l_trans_bandwidth=1, h_trans_bandwidth=1
    data_noise = raw.copy().filter(8,16,l_trans_bandwidth=1, h_trans_bandwidth=1).filter(14,10,l_trans_bandwidth=1, h_trans_bandwidth=1)._data
    ...
    for ix_comp in range(5):
        psd,freqs = psd_array_welch(D[ix_comp]@data_broad,raw.info['sfreq'],fmin=1,fmax=30,n_fft=int(3*raw.info['sfreq']))
...
def plot_forward_model(raw, forward_model):
    raw.set_montage(make_standard_montage('easycap-M1'),match_case=False)
    data_broad = raw.copy().filter(1,30,l_trans_bandwidth=1, h_trans_bandwidth=1)._data #,l_trans_bandwidth=1, h_trans_bandwidth=1
    psd, freqs = psd_array_welch(forward_model @ data_broad, raw.info['sfreq'], fmin=1, fmax=30,
...
def get_ssd_forward_model_old(raw): #SSD forward model - SNR
    ...
    #data_beta
    data_broad = raw.copy().filter(1,30)._data #,l_trans_bandwidth=1, h_trans_bandwidth=1
    data_noise = raw.copy().filter(1,30).filter(14,10,l_trans_bandwidth=1, h_trans_bandwidth=1)._data
    A = np.cov(data_alpha)#+np.cov(data_beta)
    B = np.cov(data_noise)
    evals, evecs = linalg.eig(A,B)
    ix = np.argsort(evals)[::-1]
...
def get_cca_forward_model(raw):
    ...
    data_alpha = raw.copy().filter(10,14,l_trans_bandwidth=1,h_trans_bandwidth=1)._data
    data_beta = raw.copy().filter(20,28,l_trans_bandwidth=1,h_trans_bandwidth=1)._data#.apply_hilbert(envelope=True).filter(10,14,l_trans_bandwidth=1,h_trans_bandwidth=1)._data
    data_noise = raw.copy().filter(8,16,l_trans_bandwidth=1, h_trans_bandwidth=1).filter(14,10,l_trans_bandwidth=1, h_trans_bandwidth=1)._data
    ...
    for ix_comp in range(5):
        ...
        psd, freqs = psd_array_welch(data_broad@M[:, ix_comp], raw.info['sfreq'], fmin=1, fmax=30, n_fft=int(3 * raw.info['sfreq']))
...
root = tk.Tk()
root.withdraw()
folder_path = filedialog.askdirectory(initialdir = "C:\\Users\\Annalisa Colucci\\Desktop\\\BCI_softwares\\BeamBCI-ClosedLoopStimulation\\Data\\Force Study Taisiia")
...
forward_model = loadmat('{}\\P_TARGET_64.mat'.format(folder_path))['P_TARGET_64'].squeeze()
